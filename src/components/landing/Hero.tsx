  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        <span className="text-[9px] font-semibold uppercase tracking-[0.5px] text-[var(--txt-3)]">
          {label}
        </span>
        <span className="text-[9px] font-semibold text-[var(--txt-2)]">{pct}%</span>
      </div>
      <div className="h-[3px] overflow-hidden rounded-full bg-[var(--bg-subtle-2)]">
        <div
    ...
    <div className="relative flex min-w-[240px] flex-1 flex-col gap-3 p-4 sm:min-w-0">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <div className="truncate text-[13px] font-bold text-[var(--txt)]">
            {name}
          </div>
      ...
      <div className="flex items-end justify-between">
        ...
        <div className="text-[10px] font-semibold uppercase tracking-[0.6px] text-[var(--txt-2)]">
          Grade
        </div>
      </div>
    ...
    <section className="relative overflow-hidden bg-[var(--bg-base)]">
      ...
      <div className="relative min-h-[100svh]">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(color-mix(in srgb, var(--green) 4%, transparent) 1px, transparent 1px), linear-gradient(90deg, color-mix(in srgb, var(--green) 4%, transparent) 1px, transparent 1px)",
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 30% 30%, color-mix(in srgb, var(--green) 35%, transparent), transparent 62%)",
            opacity: 0.7,
          }}
        />
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 30% 30%, color-mix(in srgb, var(--blue) 28%, transparent), transparent 62%)",
            opacity: 0.7,
          }}
        />
        <div
        ...
        <div className="relative mx-auto w-full max-w-6xl px-4 pb-16 pt-16 sm:px-6 sm:pt-20">
          ...
          >
            <motion.div variants={fadeUp} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}>
              <div className="inline-flex items-center gap-2 rounded-full border border-[var(--green-border)] bg-[var(--green-light)] px-4 py-2 text-[12px] font-semibold text-[var(--green)]">
                <span className="h-2 w-2 rounded-full bg-[var(--green)] shadow-[var(--shadow-glow-g)] animate-pulse" />
            ...
            <motion.h1
              ...
              className="mt-6 text-balance text-[44px] font-black tracking-[-2px] text-[var(--txt)] sm:text-[64px]"
            >
            ...
            <motion.p
              variants={fadeUp}
              transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
              className="mt-4 max-w-[480px] text-[16px] leading-relaxed text-[var(--txt-2)]"
            >
            ...
            >
              <div
                className="group flex w-full items-end gap-3 rounded-[14px] border border-[var(--border-md)] bg-[var(--bg-card)] px-4 py-[14px] transition-[box-shadow,border-color] duration-200 ease-in-out hover:border-[var(--green-border)] focus-within:border-[var(--green-border)] focus-within:shadow-[0_0_0_3px_color-mix(in_srgb,var(--green)_12%,transparent),0_0_20px_color-mix(in_srgb,var(--green)_8%,transparent)] sm:gap-4"
              >
                <div className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-[var(--green-light)]">
                  <span className="text-[14px] font-light leading-none text-[var(--green)]">
                    ✦
                  </span>
                </div>
                <textarea
                  ...
                  disabled={isLoading}
                  className="min-h-[24px] min-w-0 w-full resize-none border-0 bg-transparent py-1 text-left text-[14px] leading-relaxed text-[var(--txt)] outline-none placeholder:text-[var(--txt-3)]"
                />
                <button
                  type="button"
              ...
              <div className="mt-2 flex w-full flex-wrap justify-center gap-2">
                {EXAMPLE_PILLS.map((pill) => (
                  <button
                    ...
                    className="rounded-full border border-[var(--border)] bg-[var(--bg-card)] px-3 py-1.5 text-left text-[12px] text-[var(--txt-2)] transition hover:border-[var(--green-border)] hover:bg-[var(--green-light)] hover:text-[var(--green)]"
                  >
                    {pill}
                  </button>
                ))}
            ...
            {showResultsBlock ? (
              ...
              >
                <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--green)]">
                  Guru says:
                </div>

                {error ? (
                  <div className="mt-3 rounded-xl border border-[color-mix(in_srgb,var(--orange)_25%,transparent)] bg-[var(--orange-light)] px-4 py-3 text-[13px] text-[var(--orange)]">
                    {error}
                  </div>
                ) : null}
                ...
                {!isLoading && (response || error) ? (
                  <div className="mt-3">
                    <Link
                      href="/search"
                      className="inline-flex items-center gap-1 text-[14px] font-semibold text-[var(--orange)] underline-offset-4 transition hover:text-[var(--orange)] hover:underline"
                    >
                      Want more? →
                    </Link>
                  </div>
            ...
            >
              {[
                { n: "2.8M+", t: "plays tracked", c: "var(--green)" },
                { n: "1,847", t: "players profiled", c: "var(--orange)" },
                { n: "340+", t: "advanced metrics", c: "var(--blue)" },
                { n: "AI-powered", t: "search", c: "var(--green)" },
                { n: "1999–2024", t: "data", c: "var(--blue)" },
              ].map((chip) => (
                <div
                  key={chip.n}
                  className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--bg-card)] px-3 py-2 text-[12px] text-[var(--txt-2)]"
                >
                  <span className="font-bold" style={{ color: chip.c }}>
                    {chip.n}
                  </span>
                  <span className="text-[var(--txt-2)]">{chip.t}</span>
                </div>
              ))}
            </motion.div>
            ...
            >
              <div className="overflow-hidden rounded-[14px] border border-[var(--border)] bg-[var(--bg-card)] shadow-[var(--shadow-glow-soft)]">
                <div className="flex items-center gap-3 border-b border-[var(--border)] bg-[var(--bg-card2)] px-4 py-3">
                  ...
                  <div className="ml-2 flex flex-1 items-center justify-center">
                    <div className="flex w-full max-w-[420px] items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--bg-card)] px-4 py-1.5 text-[12px] text-[var(--txt-2)]">
                ...
                <div className="p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <div className="flex flex-1 items-center gap-3 rounded-[14px] border border-[var(--border)] bg-[var(--bg-card2)] px-4 py-3">
                      <div className="grid h-8 w-8 place-items-center rounded-[10px] bg-[var(--green-light)] text-[var(--green)]">
                        ✦
                      </div>
                      <div className="flex min-w-0 flex-1 items-center gap-1 text-[13px] text-[var(--txt)]">
                        <span className="truncate">
                          Show me top WRs by YPRR in 2024
                        </span>
                        <span className="h-[14px] w-[2px] animate-pulse bg-[var(--green)]" />
                      </div>
                    </div>
                    <a
                      href="/search"
                      className="inline-flex h-11 items-center justify-center rounded-full bg-[var(--green)] px-5 text-[13px] font-semibold text-[var(--on-green)] shadow-[var(--shadow-glow-g)] transition hover:brightness-110"
                    >
                      Ask the Guru ✦
                    </a>
                  </div>
